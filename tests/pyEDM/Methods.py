...
def CCM( args, source = Source.Python ):
    ...
    #create stats arr and write to file
    import csv
    stats_header = ["lib_size","ρ "+args.columns[0],"ρ "+args.target]
    stats_arr = [stats_header]
    for lib_size in range( start, stop + 1, increment ) :
        ...
        col_targ_rho = round(col_targ_rho, 4)
        targ_col_rho = round(targ_col_rho, 4)

        stats_arr.append([lib_size,col_targ_rho,targ_col_rho])


    if args.outputFile:
        with open(args.path+args.outputFile,"w+") as stats_output_csv:
            csvWriter = csv.writer(stats_output_csv,delimiter=',')
            csvWriter.writerows(stats_arr)
